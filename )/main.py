...
if os.name == "nt":
    ctypes.windll.kernel32.SetConsoleTitleW(f".gg/dwcboost")
else:
    pass
...
keyauthapp = api(
    name = "Boosttool",
    ownerid = "Jr85PdkROE",
    secret = "44c0cb24e8572b3b957ab40fa3ae482611900d011537dbde4619a0720a510bf7",
    version = "1.0",
...
def menu():
    print(fade.blackwhite("""
              ...
              ╚═════╝░░░░╚═╝░░░╚═╝░░░╚════╝░  ╚═════╝░░╚════╝░░╚════╝░╚═════╝░░░░╚═╝░░░
                         ...
                         [1] Boost:.gg/dwcboost       [2] Stock:.gg/dwcboost    
"""))
    
    choice = input(f"{Fore.WHITE}> ")
    
    if choice == "1":
        invite = getinviteCode(input(f"{Fore.WHITE}> {Fore.LIGHTBLACK_EX}Invite:{Fore.RESET} "))
        amount = input(f"{Fore.WHITE}> {Fore.LIGHTBLACK_EX}Boosts:{Fore.RESET} ")
        ...
        print()
        success(f"Successfuly Boosted {Fore.LIGHTBLACK_EX}duration={Fore.WHITE}{round(end - start, 2)}s {Fore.LIGHTBLACK_EX}success={Fore.WHITE}{len(variables.success_tokens)} {Fore.LIGHTBLACK_EX}failed={Fore.WHITE}{len(variables.failed_tokens)}")
        time.sleep(3)
        os.system('cls')
        menu()
    ...
    if choice == "2":
        ...
        print(f'{Fore.LIGHTBLACK_EX}1 Month Server Boosts: {len(open("assets/1m_tokens.txt", "r").readlines())*2}{Fore.RESET}')
        print()
        print(f'{Fore.LIGHTBLACK_EX}3 Month Nitro Tokens: {len(open("assets/3m_tokens.txt", "r").readlines())}{Fore.RESET}')
        print(f'{Fore.LIGHTBLACK_EX}3 Month Server Boosts: {len(open("assets/3m_tokens.txt", "r").readlines())*2}{Fore.RESET}')
        print()
        time.sleep(3)
        os.system('cls')
        menu()
            ...
            if len(boost_data.json()) != 0:
                ...
                if join_outcome:
                    ...
                    if config["change_server_nick"]:
                        ...
                        if changed:
                            success(f"Successfully changed bio!")
                        else:
